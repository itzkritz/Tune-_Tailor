...
function predictGenreFromText(text) {
  ...
  // Automatic Language Detection (Fallback if explicitly not stated in text)
  const detected = lngDetector.detect(text, 1); 
  if (detected && detected.length > 0) {
    const primaryLanguage = detected[0][0]; // e.g. 'spanish' or 'hindi' or 'english'
    
    // Convert to target modifier if supported and not already present
    if (primaryLanguage === 'hindi' && !modifiers.includes('hindi')) {
        modifiers.push('hindi bollywood');
    } else if (primaryLanguage === 'spanish' && !modifiers.includes('spanish')) {
        modifiers.push('spanish latin');
    }
  }
